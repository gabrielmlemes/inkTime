import Card from '@/app/(public)/_components/hero/card';
...
const Professionals = async () => {
  ...
  return (
    <section className="bg-background py-20 lg:py-24" id="professionals">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-12 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
          {tattooArtists.map((artist) => (
            <Card key={artist.name} artist={artist} studio={studio} />
          ))}
        </div>
      </div>
    </section>
